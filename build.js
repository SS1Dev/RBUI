...
function UIFramework.CreateCard(config)
    ...
    Utilities.ApplyCorner(card, Theme.BorderRadius.LG)
    Utilities.ApplyStroke(card, { Color = Theme.Colors.SurfaceBorder, Thickness = 1 })
    ...
    if config.AutoLayout ~= false then
        Utilities.ApplyListLayout(card, { Padding = config.LayoutPadding or Theme.Spacing.SM })
    end
    return card
end
